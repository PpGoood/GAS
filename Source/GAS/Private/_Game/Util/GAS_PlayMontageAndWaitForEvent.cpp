...
#include "_Game/Util/GAS_PlayMontageAndWaitForEvent.h"
...
UMyAbilitySystemComponent* UGAS_PlayMontageAndWaitForEvent::GetTargetASC()
{
...
UGAS_PlayMontageAndWaitForEvent* UGAS_PlayMontageAndWaitForEvent::PlayMontageAndWaitForEvent(UGameplayAbility* OwningAbility,
...
void UGAS_PlayMontageAndWaitForEvent::Activate()
{
	...
	UMyAbilitySystemComponent* RPGAbilitySystemComponent = GetTargetASC();
	...
	if (RPGAbilitySystemComponent)
	{
		...
		if (AnimInstance != nullptr)
		{
			...
			EventHandle = RPGAbilitySystemComponent->AddGameplayEventTagContainerDelegate(EventTags, FGameplayEventTagMulticastDelegate::FDelegate::CreateUObject(this, &UGAS_PlayMontageAndWaitForEvent::OnGameplayEvent));
		...
		{
			ABILITY_LOG(Warning, TEXT("UGAS_PlayMontageAndWaitForEvent call to PlayMontage failed!"));
		}
	}
	else
	{
		ABILITY_LOG(Warning, TEXT("UGAS_PlayMontageAndWaitForEvent called on invalid AbilitySystemComponent"));
	}

	if (!bPlayedMontage)
	{
		ABILITY_LOG(Warning, TEXT("UGAS_PlayMontageAndWaitForEvent called in Ability %s failed to play montage %s; Task Instance Name %s."), *Ability->GetName(), *GetNameSafe(MontageToPlay),*InstanceName.ToString());
		if (ShouldBroadcastAbilityTaskDelegates())
		{
			OnCancelled.Broadcast(FGameplayTag(), FGameplayEventData());
		}
